Recoil:RegisterMode("envy", function(self)
    ...
    local GroupRecoil = {
        ...
        if isArmed and IsPedShooting(plyPed) then -- Check if they are armed and dangerous (shooting)
            ...
            local weaponRecoil =
                WeaponRecoil[weapon] or GroupRecoil[GetWeapontypeGroup(weapon)] or
                {vertical = 0.1, horizontal = 0.1}

            local rightLeft = math.random(1, 4) -- Chance to move left or right

            local horizontalRecoil =
                (headingFactor) * ((weaponRecoil.horizontal or 0.1) * 10)
            ...
            if not isMoving then -- Checks if the recoil is already being vertically adjusted
                ...
                local finalRecoilTarget = (recoil * (weaponRecoil.vertical * 10)) -- Working out the target for recoil
                ...
                repeat
                    ...
                    SetGameplayCamRelativePitch(
                        GetGameplayCamRelativePitch() + (weirdRecoil and (math.random(28, 32) / 10) or 0.1),
                        0.2
                    ) -- Move the camera pitch up by 0.1
