...
LSC_Config.prices = {
    ...
    ----Wheel accessories----
    wheelaccessories = {
        -- { name = "Stock Tires", price = 0, DefaultPrice = 0},
        {
            name = "Custom Tires",
            price = 0,
            DefaultPrice = 0
        },
        --{ name = "Bulletproof Tires", price = 00, DefaultPrice = 00},
        {
...
local wait = Wait
local set_local_player_visible_locally = SetLocalPlayerVisibleLocally
local draw_marker = DrawMarker
local player_ped_id = PlayerPedId
local is_ped_sitting_in_any_vehicle = IsPedSittingInAnyVehicle
...
function Menu.new(title, header, x, y, width, height, font)
    ...
    return set_metatable(newmenu, Menu)
end
...
_DrawText = DrawText
function DrawText(text, pos, colour, fontId, scale)
    if not scale then scale = { 0.0, 0.2 } end
    colour = colour or { 128, 128, 128, 255 }
    SetTextFont(fontId or 0)
    SetTextProportional(1)
    SetTextScale(table.unpack(scale))
...
function drawInfo(text, font, x, y, width, height, scale, r, g, b, a)
    ...
    SetTextEntry("STRING")
    AddTextComponentString(text)
    _DrawText(x - width / 2.2, y - height / 2.8)
end
    ...
    createthread(function()
        ...
        while self.visible do
            ...
            if self.title_sprite then
                ...
                drawMenuText(self.title, 1, 1, tx, ty - height - 0.009, scale * 2.4, cfg.stext_color.r, cfg.stext_color.g,
                cfg.stext_color.b, cfg.stext_color.a)
                DrawRect(x, y - 0.019, width, height + 0.04, cfg.sbg_color.r, cfg.sbg_color.g, cfg.sbg_color.b, cfg.sbg_color
                .a)
            end
            y = y + height
            ty = y - height / 2.8
            ...
            --menu info
            if self.header then
                drawMenuText(menu.header:upper(), cfg.font, 0, tx, ty, scale, cfg.text_color.r, cfg.text_color.g,
                cfg.text_color.b, cfg.text_color.a)
            end
            drawRightMenuText(menu.selected .. '/' .. tablelength(menu.buttons), cfg.font, 0, x + (width / 2.1), ty, scale,
            cfg.text_color.r, cfg.text_color.g, cfg.text_color.b, cfg.text_color.a)
            ...
            for i, btn in pairs(buttons) do
                if i >= menu.from and i <= menu.to then
                    if menu.selected == i then
                        ...
                        drawMenuText(btn.name, cfg.font, 0, tx, ty, scale, cfg.stext_color.r, cfg.stext_color.g, cfg.stext_color.b,
                        cfg.stext_color.a)
                        ...
                        if btn.sprite ~= nil then
                            if btn.sprite == "garage" then
                                DrawSprite("commonmenu", "shop_garage_icon_b", x + (width / 2.4), y, scale / 10 - 0.01, scale / 10 +
                                0.01, 0.00, cfg.text_color.r, cfg.text_color.g, cfg.text_color.b, cfg.text_color.a)
                            end
                        elseif btn.price ~= nil then
                            if btn.price == 0 then
                                drawRightMenuText("", cfg.font, 0, x + (width / 2.1), ty, scale, cfg.stext_color.r, cfg.stext_color.g,
                                cfg.stext_color.b, cfg.stext_color.a)
                            else
                                if not btn.purchased then
                                    drawRightMenuText("$" .. btn.price, cfg.font, 0, x + (width / 2.1), ty, scale, cfg.stext_color.r,
                                    cfg.stext_color.g, cfg.stext_color.b, cfg.stext_color.a)
                                else
                                    drawRightMenuText("OWNED", cfg.font, 0, x + (width / 2.1), ty, scale, cfg.stext_color.r,
                                    cfg.stext_color.g, cfg.stext_color.b, cfg.stext_color.a)
                                end
                            end
                        elseif btn.menu ~= nil then
                            DrawSprite("commonmenutu", "arrowright", x + (width / 2.2), y, scale / 14 - 0.01, scale / 19 + 0.01, 0.00,
                            cfg.stext_color.r, cfg.stext_color.g, cfg.stext_color.b, cfg.stext_color.a)
                        end
                        ...
                        if btn.checkbox ~= nil then
                            if btn.checkbox then
                                DrawSprite("commonmenu", "shop_box_tickb", x + (width / 2.3), y, scale / 10, scale / 10, 0.00,
                                cfg.stext_color.r, cfg.stext_color.g, cfg.stext_color.b, cfg.stext_color.a)
                            else
                                DrawSprite("commonmenu", "shop_box_blankb", x + (width / 2.3), y, scale / 10, scale / 10, 0.00,
                                cfg.stext_color.r, cfg.stext_color.g, cfg.stext_color.b, cfg.stext_color.a)
                            end
                        end
                        if btn.list then
                            drawRightMenuText('< ' .. btn.list[btn.listindex] .. ' >', cfg.font, 0, x + (width / 2.1), ty, scale,
                            cfg.stext_color.r, cfg.stext_color.g, cfg.stext_color.b, cfg.stext_color.a)
                        ...
                        drawMenuText(btn.name, cfg.font, 0, tx, ty, scale, cfg.text_color.r, cfg.text_color.g, cfg.text_color.b,
                        cfg.text_color.a)
                        if btn.list then
                            drawRightMenuText('< ' .. btn.list[btn.listindex] .. ' >', cfg.font, 0, x + (width / 2.1), ty, scale,
                            cfg.text_color.r, cfg.text_color.g, cfg.text_color.b, cfg.text_color.a)
                        ...
                        if btn.sprite ~= nil then
                            if btn.sprite == "garage" then
                                DrawSprite("commonmenu", "shop_garage_icon_a", x + (width / 2.4), y, scale / 10 - 0.01, scale / 10 +
                                0.01, 0.00, cfg.text_color.r, cfg.text_color.g, cfg.text_color.b, cfg.text_color.a)
                            end
                        elseif btn.price ~= nil then
                            if btn.price ~= 0 then
                                if not btn.purchased then
                                    drawRightMenuText("$" .. btn.price, cfg.font, 0, x + (width / 2.1), ty, scale, cfg.text_color.r,
                                    cfg.text_color.g, cfg.text_color.b, cfg.text_color.a)
                                else
                                    drawRightMenuText("OWNED", cfg.font, 0, x + (width / 2.1), ty, scale, cfg.text_color.r,
                                    cfg.text_color.g, cfg.text_color.b, cfg.text_color.a)
                                end
                            end
                        elseif btn.menu ~= nil then
                            DrawSprite("commonmenu", "arrowright", x + (width / 2.2), y, scale / 14 - 0.01, scale / 19 + 0.01, 0.00,
                            cfg.text_color.r, cfg.text_color.g, cfg.text_color.b, cfg.text_color.a)
                        end
                        if btn.checkbox ~= nil then
                            if btn.checkbox then
                                DrawSprite("commonmenu", "shop_box_tick", x + (width / 2.3), y, scale / 10, scale / 10, 0.00,
                                cfg.text_color.r, cfg.text_color.g, cfg.text_color.b, cfg.text_color.a)
                            else
                                DrawSprite("commonmenu", "shop_box_blank", x + (width / 2.3), y, scale / 10, scale / 10, 0.00,
                                cfg.text_color.r, cfg.text_color.g, cfg.text_color.b, cfg.text_color.a)
                            end
                        end
                    end
            ...
            if tablelength(buttons) > menu.maxbuttons then
                DrawRect(x, y, width, height, cfg.bg_color.r, cfg.bg_color.g, cfg.bg_color.b, 255)
                DrawSprite("commonmenu", "shop_arrows_upanddown", x, y, scale / 13, scale / 9, 0.00, cfg.text_color.r,
                cfg.text_color.g, cfg.text_color.b, cfg.text_color.a)
                y = y + height
                ty = y - height / 2.8
            end
            if self.notification ~= nil then
                ...
                drawInfo(self.notification, cfg.font, x, y, width, height, scale, cfg.text_color.r, cfg.text_color.g,
                cfg.text_color.b, cfg.text_color.a)
            elseif menu.buttons[menu.selected] and menu.buttons[menu.selected].info ~= nil then
                DrawRect(x, y - height + y - ty + 0.0055, width, 0.0035, cfg.sbg_color.r, cfg.sbg_color.g, cfg.sbg_color.b, 255)
                drawInfo(menu.buttons[menu.selected].info, cfg.font, x, y, width, height, scale, cfg.text_color.r,
                cfg.text_color.g, cfg.text_color.b, cfg.text_color.a)
            end
...
local garages = {
    [1]=    { isBusy = false, locked = false, camera = {x = 242.57, y = -1409.98, z = 30.59-1, heading = 0.79}, driveout = {x = 242.57, y = -1409.98, z = 30.59-1, heading = 0.79}, drivein = {x = 242.57, y = -1409.98, z = 30.59-1, heading = 0.79}, outside = {x = 242.57, y = -1409.98, z = 30.59-1, heading = 0.79}, inside = {x = 242.57, y = -1409.98, z = 30.59-1, heading = 0.79}
}}
...
local function SetupModPrices()
    local vehicle = GetVehiclePedIsIn(player_ped_id())
...
--Setup main menu
local LSCMenu = Menu.new("Grove Customs", "CATEGORIES", 0.16, 0.13, 0.24, 0.36, 0, {
    255,
    255,
    255,
    255
...
local function DriveInGarage()
    ...
    if not is_ped_sitting_in_any_vehicle(ped) then
        return
    end
    --Lock the garage
    --StartFade()
    local veh = get_vehicle_ped_is_using(ped)
    if not get_ped_in_vehicle_seat(veh, -1) == ped then
        return
    end
    ...
    if does_entity_exist(veh) then
        --Set menu title
        LSCMenu:setTitle("Bennys Customs")
        ...
        if type(LSC_Config.menu.position) == 'table' then
            LSCMenu.config.position = {
                x = LSC_Config.menu.position.x,
                y = LSC_Config.menu.position.y
            }
        ...
        if type(LSC_Config.menu.theme) == "table" then
            LSCMenu:setColors(LSC_Config.menu.theme.text_color, LSC_Config.menu.theme.stext_color,
            LSC_Config.menu.theme.bg_color, LSC_Config.menu.theme.sbg_color)
        ...
        LSCMenu:addPurchase("Repair Vehicle", round(250 + 150 * damage, 0), "Full Vehicle Repair")
        ...
        for n, c in pairs(LSC_Config.prices.cham2.colors) do
            local btn = scol.Chameleon:addPurchase(c.name, LSC_Config.prices.cham2.price)
            btn.colorindex = c.colorindex
            if btn.colorindex == myveh.color[2] then
                btn.purchased = true
            end
        ...
        for n, c in pairs(LSC_Config.prices.chrome2.colors) do
            local btn = perl.Chrome:addPurchase(c.name, LSC_Config.prices.chrome2.price)
            btn.colorindex = c.colorindex
        ...
        for n, c in pairs(LSC_Config.prices.classic2.colors) do
            local btn = perl.Classic:addPurchase(c.name, LSC_Config.prices.classic2.price)
            btn.colorindex = c.colorindex
            if btn.colorindex == myveh.color[3] then
                btn.purchased = true
            end
        ...
        for n, c in pairs(LSC_Config.prices.matte2.colors) do
            local btn = perl.Matte:addPurchase(c.name, LSC_Config.prices.matte2.price)
            btn.colorindex = c.colorindex
            if btn.colorindex == myveh.color[3] then
                btn.purchased = true
            end
        ...
        for n, c in pairs(LSC_Config.prices.metallic2.colors) do
            local btn = perl.Metallic:addPurchase(c.name, LSC_Config.prices.metallic2.price)
            btn.colorindex = c.colorindex
        ...
        for n, c in pairs(LSC_Config.prices.metal2.colors) do
            local btn = perl.Metals:addPurchase(c.name, LSC_Config.prices.metal2.price)
            btn.colorindex = c.colorindex
            if btn.colorindex == myveh.color[3] then
                btn.purchased = true
            end
        ...
        if IsThisModelABike(GetEntityModel(veh)) then
            ...
            for i = 51, 112, 1 do
                ...
                local mname = tostring(GetLabelText(lbl))
                local btn = customwheels:addPurchase(mname, 0)
                btn.wtype = 0
                btn.modtype = 23
                btn.mod = i
            end
...
local function interactListener()
    createthread(function()
        while interactCheck do
...
function LSCMenu:onSelectedIndexChanged(name, button)
    ...
    if m == "chameleon" or m == "chrome" or m == "classic" or m == "matte" or m == "metals" then
...
function LSCMenu:onButtonSelected(name, button)
    ...
    if button.price then
        if not button.purchased then
            TriggerEvent("erotic-bennys:buttonSelected", name, button, true)
        end
    end
end

--So we get the button back from server +    bool that determines if we can prchase specific item or not
RegisterNetEvent("erotic-bennys:buttonSelected")
AddEventHandler("erotic-bennys:buttonSelected", function(name, button, canpurchase)
    name = name:lower()
    local m = LSCMenu.currentmenu
    local price = button.price or 0
    local veh = myveh.vehicle
    ...
    if mname == "chameleon" or mname == "chrome" or mname == "classic" or mname == "matte" or mname == "metals" then
...
function CheckPurchases(m)
    name = m.name:lower()
    if name == "chameleon" or name == "chrome" or name == "classic" or name == "matte" or name == "metals" then
        if m.parent == "Primary color" then
...
function CanPurchase(price, canpurchase)
    if canpurchase then
        if LSCMenu.currentmenu == "main" then
            LSCMenu:showNotification("Vehicle has been repaired.")
        else
...
createthread(function()
    while true do
        ...
        if exports["noob"]:inSafeZone() then
            ...
            if not inside then
                local ped = player_ped_id()
                if is_ped_sitting_in_any_vehicle(ped) then
                    local veh = get_vehicle_ped_is_using(ped)
                    if does_entity_exist(veh) and get_ped_in_vehicle_seat(veh, -1) == ped then
                        local distance = get_distance_between_coords(coords, get_entity_coords(ped))
                        if distance <= 7.5 then
                            sleepTimer = 1
                            if is_control_just_pressed(1, 38) then
                                SetupModPrices()
                                inside = true
                                currentgarage = 1
                                editCount = 0
                        ...
                        elseif distance <= 20.0 then
                            sleepTimer = 1
                            draw_marker(21, coords.x, coords.y, coords.z + 0.7, vector3(0,0,0), vector3(0,0,0), vector3(1.1, 1.1, 0.9), 201, 116, 255, 125, false, false, false, true, false, false, false)
                        else
                            sleepTimer = 2000
                        end
                    end
            ...
            if inside then
                sleepTimer = 2000
                set_local_player_visible_locally(1)
            end
