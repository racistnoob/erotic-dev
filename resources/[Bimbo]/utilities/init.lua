-- utilities <https://github.com/overextended/utilities>
...
local export = exports[utilities]

if not GetResourceState(utilities):find('start') then
	error('^1utilities should be started before this resource.^0', 2)
end
...
local function loadModule(self, module)
	...
	local chunk = LoadResourceFile(utilities, ('%s/%s.lua'):format(dir, context))
	local shared = LoadResourceFile(utilities, ('%s/shared.lua'):format(dir))
	...
	if chunk then
		local fn, err = load(chunk, ('@@utilities/%s/%s.lua'):format(module, context))
...
lib = setmetatable({
	...
	onCache = function(key, cb)
		AddEventHandler(('utilities:cache:%s'):format(key), cb)
	end
}, {
	__index = call,
	__call = call,
...
if context == 'client' then
	setmetatable(cache, {
		__index = function(self, key)
			AddEventHandler(('utilities:cache:%s'):format(key), function(value)
				self[key] = value
			end)
