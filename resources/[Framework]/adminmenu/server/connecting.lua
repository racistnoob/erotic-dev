...
if banCheck then
    AddEventHandler("playerConnecting", function(name, setKickReason, d)
        ...
        local bans, isBanned, isPerma = exports['adminmenu']:Bans(), false, false
            ...
            if BanInfo.steamid == steamid or BanInfo.discord == discord or BanInfo.license == license or BanInfo.ip == ip or BanInfo.fivem == fivem then
                ...
                elseif BanInfo.date - os.time() > 0 then
                    ...
                    if GetResourceState("oxmysql") == "started" then
                        exports.oxmysql:execute("DELETE FROM bannedplayers WHERE id=:id", { id = BanInfo.id }, function(result) if result > 0 then exports['adminmenu']:RefreshBans() end end)
                    elseif GetResourceState("mysql-async") == "started" then
                        MySQL.Async.execute("DELETE FROM bannedplayers WHERE id=@id", { id = BanInfo.id }, function(result) if result > 0 then exports['adminmenu']:RefreshBans() end end)
                    end
                    break
                end
            end
