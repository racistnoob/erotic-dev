...
AddEventHandler('erp_adminmenu:kickPlayer', function(target, reason)
    if IsPlayerAceAllowed(source, 'erotic.mod') and target and reason then
...
AddEventHandler('erp_adminmenu:spectate', function(target, on, src)
    local source = src
    if IsPlayerAceAllowed(source, 'erotic.mod') and target then
...
RegisterCommand("goto", function(source, args, rawCommand)
	if not IsPlayerAceAllowed(source, 'erotic.mod') then return end
	local target = args[1]
...
RegisterCommand("tome", function(source, args, rawCommand)
	if not IsPlayerAceAllowed(source, 'erotic.mod') then
		return
	end
	local target = args[1]
	if target ~= nil and target ~= source then
...
AddEventHandler('erp_adminmenu:unbanPlayer', function(banid)
    local source = source
    local banid = banid
    if IsPlayerAceAllowed(source, 'erotic.mod') and banid then
		if GetResourceState("oxmysql") == "started" then
...
AddEventHandler('erp_adminmenu:spawnVehicle', function(coords, vehicle)
    local src = source
		if IsPlayerAceAllowed(src, 'erotic.mod') then
