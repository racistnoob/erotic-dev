...
AddEventHandler('echorp:fetchcharacters', function()
	...
	local knownCharacters = exports.oxmysql:fetchSync("SELECT id, cash, bank, firstname, lastname, dateofbirth, gender FROM users WHERE identifier=:identifier AND deleted='0'", { identifier = identifier })
...
AddEventHandler('echorp:selectCharacter', function(cid) 
  ...
  if players[playerId] or playersCid[cid] then
	...
	if identifier then
		exports.oxmysql:fetch('SELECT `cash`, `bank`, `job`, `job_grade`, `duty`, `firstname`, `lastname`, `gender`, `phone_number`, `jail_time`, `twitterhandle` FROM `users` WHERE `id`=:id LIMIT 1', { id = cid }, function(charInfo)
			if charInfo then
											...
											exports.oxmysql:execute("UPDATE users SET bank = bank+:amount WHERE id=:id", {amount = amount, id = target}, function(newResult)
													if newResult then
															...
															if targetPly then
																	...
																	TriggerClientEvent('drp-notifications:client:SendAlert', targetPly['source'], { type = 'inform', text = 'You were transferred $'..amount..' from '..player['fullname'], length = 5000 })
															end
													end 
											end)
									else
...
AddEventHandler('AdjustCash', function(playerId, amount, isCid, cb) 
	local id = playerId
	if isCid == nil or not isCid then id = exports['drp']:GetPlayerFromId(playerId).cid end
	if type(amount) == 'string' then amount = tonumber(amount) end if type(playerId) == 'string' then playerId = tonumber(playerId) end
