    ...
    while true do
        ...
        if drunkLevel >= 37 then
            ...
            if math.random(100) >= 90 then
                TriggerServerEvent('drp-status:applyStatus', 'Breath smells like Alcohol')
            end
            if math.random(100) >= 90 then
                TriggerServerEvent('drp-status:applyStatus', 'Uncoordinated')
            end
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, durability)
    ...
    if durability ~= nil then
        if tonumber(durability) <= 1 then
            exports['drp-notifications']:SendAlert('inform', 'This item is too worn out to be used.', 3000)
        ...
        if ItemInfo and ItemInfo['information'] then
            ...
            if dist < 10.0 then
                ...
                if DNAInfo['id'] == nil then
                    ...
                    local finished = exports["drp-progressbar"]:taskBar(5000, msg)
                    if (finished == 100) then
                        DNAInfo = { ItemID = ItemInfo['id'], information = json.decode(ItemInfo['information']) }
                        CreateThread(function()
                            exports['drp-notifications']:SendAlert('inform', 'DNA scanning... results will appear in 1 hour (60s)', 5000)
                            Wait(60000)
                            TriggerServerEvent('drp-inventory:runEvidence', DNAInfo['information'])
                        end)
                    end
                end
            else
                exports['drp-notifications']:SendAlert('inform', 'Too far from the lab.', 5000)
            end 
        end 
    end

    --[[if itemid == 'ipodnano' then
        TriggerEvent('drp-soundcloud:useItem', ItemInfo)
    end]]
    ...
    if itemid == 'cottonswab' then
        if ItemInfo and (ItemInfo['information'] ~= "{}") then
            ...
            if dist < 10.0 then
                local finished = exports["drp-progressbar"]:taskBar(5000,"Scanning Cotton Swab")
                if finished == 100 then
                    TriggerServerEvent('drp-inventory:finishCottonSwab', ItemInfo['id'], json.decode(ItemInfo['information']))
                end 
            end 
        else
            t, distance = GetClosestPlayer()
            if(distance ~= -1 and distance < 5) then
                TriggerServerEvent('drp-inventory:runCottonSwab', ItemInfo['id'], GetPlayerServerId(t))
            end
        end
    end
    ...
    if itemid == "-72657034" then
        ...
        if not HasPedGotWeapon(plyPed, `GADGET_PARACHUTE`, false) then
            ...
            if HasPedGotWeapon(plyPed, `GADGET_PARACHUTE`, false) == 1 then
                ...
                exports['drp-notifications']:SendAlert('inform', 'Parachute applied.', 3000)
            else
                exports['drp-notifications']:SendAlert('inform', 'Parachute not applied.', 3000)
            end
        else
            exports['drp-notifications']:SendAlert('inform', 'You already have a parachute applied.', 4000)
        end
        justUsed, retardCounter, lastCounter = false, 0, 0
        canOpenInventory = true
        return
        ...
        if targetVehicle ~= 0 then
            if not GetIsVehicleEngineRunning(targetVehicle) then
                if #(GetEntityCoords(targetVehicle) - coordA) < 3.0 then
                    local fuelLevel = exports["drp-fuel"]:GetFuel(targetVehicle)
                    ...
                    local finished = exports["drp-progressbar"]:taskBar(timer,"Fueling",false,false)
                    if (finished == 100) then
                        isFueling = false
                        if hasEnoughOfItem(itemid,1,false) then 
                            exports["drp-fuel"]:SetFuel(targetVehicle, 100)
                            TriggerEvent("inventory:removeItem",itemid, 1)
                        end                       
                    else
                        isFueling = false
                        exports['drp-notifications']:SendAlert('error', 'Failed to fuel the vehicle', 5000)
                    end
                else
                    exports['drp-notifications']:SendAlert('inform', 'The jerrycan is not long enough to reach the vehicle', 5000)
                end 
            else 
                exports['drp-notifications']:SendAlert('inform', 'Please turn the engine off.', 5000)
            end 
        else 
    ...
    if itemid == 'cigarette' then
        if hasEnoughOfItem('lighter',1,false) then
            remove = true
            local finished = exports["drp-progressbar"]:taskBar(1000,"Lighting Up")
            if (finished == 100) then
                ...
                CreateThread(function()    
                    ...
                    while true do
                        ...
                        if shouldEnd then
                            exports['drp-notifications']:SendAlert('inform', 'You finished your cigarette.', 6000)
                            ...
                            if math.random(100) >= 90 then
                                ...
                                if closestPlayers then
                                    for k,v in pairs(closestPlayers) do
                                        TriggerServerEvent('drp-status:applyStatus', 'Smells like smoke', GetPlayerServerId(v))
                                    end
                                end
                            end
                        else
                            exports['drp-notifications']:SendAlert('inform', 'You dropped your cigarette before you could finish it.', 6000)
                            return
                        end
                    end
                end)            
            ...
            exports['drp-notifications']:SendAlert('inform', 'Missing lighter.')
        end
        
    end

    if (itemid == 'washkit') then TriggerEvent('drp-washkit:onUse', itemid) end
    ...
    elseif itemid == 'thermal_charge' then
        --[[TriggerServerEvent('drp-robbery:thermiteBank')
        TriggerServerEvent('drp-vault:thermiteBox')
        TriggerEvent('drp-vault:thermDoor')]]
        TriggerEvent('drp-robberies:truck:itemuse', itemid)
    elseif itemid == 'laptop_h' then
        TriggerEvent('drp-powerplant:itemused', itemid)
        TriggerEvent('drp-robbery:securityBlueUsed')
    end
    ...
    if itemid == 'blackcard' then
        TriggerEvent('drp-vault:blackCard')
    end
    ...
    if itemid == "fishingrod" then
        TriggerEvent('drp-fishing:findLoc', false)
    end

    if itemid == "fishbait" then
        TriggerEvent('drp-fishing:applyBait', GetPlayerServerId(PlayerId()))
    end
    ...
    if itemid == 'shovel' then
        ...
        if hit == 1 then
            ...
            if DigSoil[material] then
                ...
                local finished = exports["drp-skillbar"]:taskBar(math.random(5000, 10000),math.random(4,6))
                if finished == 100 then
                    TriggerEvent('player:receiveItem', 'bagofsoil', math.random(1, 3))
                    ClearPedTasksImmediately(PlayerPedId())
                end
            else
                exports['drp-notifications']:SendAlert('inform', 'Unable to dig here.', 4000)
            end
        end
    end
    ...
    if (itemid == "weed_seed") or (itemid == 'weed_seed_h') or (itemid == 'phoenix_seed') then
        ...
        TriggerServerEvent('drp-status:applyStatus', 'Smells Like Marijuana')
    end
    ...
    if (itemid == "lighter") then
        ...
        local finished = exports["drp-progressbar"]:taskBar(2000,"Starting Fire",false,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid,1,false) then
                TriggerEvent('drp-weed:destroyPlant')
                CreateThread(function()
                    local fire = StartScriptFire(x,y,groundZ,math.random(10,20),true)
                    TriggerServerEvent('drp-status:applyStatus', 'Smells like smoke')
    ...
    if (itemid == "advlockpick") then
        TriggerEvent('houseRobberies:attempt', 1, false, function(res) if not res then TriggerServerEvent('drp-storerobberies:findRegister', ItemInfo.id) end end)
    end

    if itemid == 'bluecard' then
        TriggerEvent('drp-robberies:vangelico:usecard', ItemInfo.id)
    end

    if itemid == 'commonsc' then
        if IsPedInAnyVehicle(PlayerPedId(), false) then
            exports['drp-notifications']:SendAlert('inform', 'Unable to do this in a vehicle', 5000)
        else
            TriggerEvent('drp-scratchcard:useItem', 'common', itemid)
        end
    elseif itemid == 'raresc' then
        if IsPedInAnyVehicle(PlayerPedId(), false) then
            exports['drp-notifications']:SendAlert('inform', 'Unable to do this in a vehicle', 5000)
        else
            TriggerEvent('drp-scratchcard:useItem', 'rare', itemid)
        end
    elseif itemid == 'epicsc' then
        if IsPedInAnyVehicle(PlayerPedId(), false) then
            exports['drp-notifications']:SendAlert('inform', 'Unable to do this in a vehicle', 5000)
        else
            TriggerEvent('drp-scratchcard:useItem', 'epic', itemid)
        end
    elseif itemid == 'legendarysc' then
        if IsPedInAnyVehicle(PlayerPedId(), false) then
            exports['drp-notifications']:SendAlert('inform', 'Unable to do this in a vehicle', 5000)
        else
            TriggerEvent('drp-scratchcard:useItem', 'legendary', itemid)
        end
    end

    if itemid == 'greencard' then
        TriggerServerEvent('drp-robberies:findFleeca:securitypanel', ItemInfo.id)
    elseif itemid == 'laptop' then
        TriggerServerEvent('drp-robberies:findFleeca:vault')
    elseif itemid == 'fakeplate' then
        ...
        if DoesEntityExist(targetVehicle) then
            if GetEntityType(targetVehicle) ~= 2 then
                exports['drp-notifications']:SendAlert('inform', 'Not a vehicle')
                return
            end
            TriggerEvent('dp:playEmote', "mechanic3")
            local finished = exports["drp-progressbar"]:taskBar(math.random(1500, 3000),"Applying Plate",false,false,playerVeh)
            if (finished == 100) then
                if hasEnoughOfItem(itemid,1,false) then
    ...
    if itemid == "notepad" then
        TriggerEvent('drp-notepad:OpenNotepadGui')
    end

    if (itemid == "sniperammo") then
        local finished = exports["drp-progressbar"]:taskBar(5000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid,1,false) then
                if GetSelectedPedWeapon(PlayerPedId()) ~= `WEAPON_UNARMED` then
                    TriggerEvent("actionbar:ammo",itemid,30,true)
                else
                    exports['drp-notifications']:SendAlert('inform', 'No weapon in your hand.', 4000)
                end
            end
        end
    end
    ...
    if (itemid == "heavyammo") then
        local finished = exports["drp-progressbar"]:taskBar(3000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid,1,false) then
                if GetSelectedPedWeapon(PlayerPedId()) ~= `WEAPON_UNARMED` then
    ...
    if (itemid == "pistolammo") then
        local finished = exports["drp-progressbar"]:taskBar(2000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid,1,false) then
                if GetSelectedPedWeapon(PlayerPedId()) ~= `WEAPON_UNARMED` then
                    TriggerEvent("actionbar:ammo",itemid,999,true)
                else
                    exports['drp-notifications']:SendAlert('inform', 'No weapon in your hand.', 4000)
                end
            end
    ...
    if (itemid == "radio") then
        TriggerEvent('drp-radio:openRadio')
    elseif (itemid == "policescanner") then
        local finished = exports["drp-progressbar"]:taskBar(math.random(5000, 7500),"Scanning")
        if (finished == 100) then
    ...
    if (itemid == "shotgunammo") then
        local finished = exports["drp-progressbar"]:taskBar(2500,"Reloading",false,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid,1,false) then
                if GetSelectedPedWeapon(PlayerPedId()) ~= `WEAPON_UNARMED` then
                    TriggerEvent("actionbar:ammo",itemid,50,true)
    ...
    if (itemid == "subammo") then
        local finished = exports["drp-progressbar"]:taskBar(1500,"Reloading",false,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid,1,false) then
                TriggerEvent("actionbar:ammo",itemid,999,true)
            else
    ...
    if (itemid == "HeavyArmor") then
        ...
        RequestAnimDict('clothingshirt')
        while not HasAnimDictLoaded("clothingshirt") do Citizen.Wait(100) end
        TaskPlayAnim(player,'clothingshirt', 'try_shirt_positive_d',1.0, -1, 1900, 49, 3.0, 0, 0, 0)
        CreateThread(function()
            Wait(100)
            TaskPlayAnim(player, "clothingshirt", "try_shirt_positive_d", 1.0, -1, -1, 50, 0, 0, 0, 0)
        end)
        local finished = exports["drp-progressbar"]:taskBar(math.random(6500, 7500),"Heavy Armor",false,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid,1,false) then
    ...
    if (itemid == "MedArmor") then
        ...
        local finished = exports["drp-progressbar"]:taskBar(math.random(4500, 5500),"Medium Armor",false,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid,1,false) then
                AddArmourToPed(player, 65)
                TriggerEvent("inventory:removeItem",itemid, 1)
                ClearPedTasks(player)
            end
        end
    ...
    if (itemid == "SmallArmor") then
        ...
        local finished = exports["drp-progressbar"]:taskBar(math.random(2500, 3500),"Small Armor",false,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid,1,false) then
    ...
    if (itemid == "bucket") then
        ...
        if IsPedSwimmingUnderWater(PlayerPedId()) or IsPedSwimming(PlayerPedId()) then 
            local finished = exports["drp-progressbar"]:taskBar(math.random(10000, 15000),"Filling Bucket")
            if finished == 100 then 
                if hasEnoughOfItem(itemid,1,false) then
                    TriggerEvent("inventory:removeItem", itemid, 1)
    ...
    if (itemid == "nitrous") then
        ...
        Citizen.CreateThread(function()
            while finished ~= 100 and not cancelNos do
                Citizen.Wait(100)
                if GetEntitySpeed(currentVehicle) > 11 then
                    exports["drp-progressbar"]:closeGuiFail()
                    cancelNos = true
                end
            end
        end)
        finished = exports["drp-progressbar"]:taskBar(math.random(3400, 3700), "Nitrous")
        TriggerEvent("inventory:removeItem", itemid, 1)
        if (finished == 100 and not cancelNos) then
            ...
            if IsInVehicle then
                ...
                TriggerServerEvent('nitrous:server:LoadNitrous', GetVehicleNumberPlateText(currentVehicle))
            end
        else
    ...
    if (itemid == "harness") then
        ...
        finished = exports["drp-progressbar"]:taskBar(math.random(12500,15000),"Harness")
        if (finished == 100 and not cancelHarness) then
            ...
            exports['drp-notifications']:SendAlert('inform', "You can't drive and enable drift mode at the same time.", 5000)
        end
    end
    ...
    --[[if (itemid == "drift") then
        ...
        finished = exports["drp-progressbar"]:taskBar(math.random(5000,7500),"Toggling Drift Mode")
        if (finished == 100 and not cancelDrift) then
            if hasEnoughOfItem(itemid,1,false) then
                ...
                if newdriftstate then
                    ...
                    exports['drp-notifications']:SendAlert('inform', 'Drift mode enabled.')
                    ...
                    exports['drp-notifications']:SendAlert('inform', 'Drift mode disabled.')
    ...
    if itemid == "casinodiamond" or itemid == "casinogold" or itemid == "casinoplat" then 
        if ItemInfo and ItemInfo['information'] then
            ...
            if tonumber(cid) == plyData['cid'] then 
                ...
                if shouldHighTable then TriggerEvent('drp-casino:hightable', true) end
            else
                exports['drp-notifications']:SendAlert('inform', 'This card is not registered to you.')
            end
        else
            exports['drp-notifications']:SendAlert('inform', 'Seems like an invalid card.')
        end
    end
    ...
    if (itemid == "cigar") then
        local finished = exports["drp-progressbar"]:taskBar(1000,"Lighting Up",false,false,playerVeh)
        if (finished == 100) then
            Wait(300)
    ...
    if (itemid == "oxygentank") then
        ...
        local skillbar =  exports["drp-skillbar"]:taskBar(math.random(7500, 10000),math.random(9,12))
        if skillbar == 100 then
            ...
            local finished = exports["drp-progressbar"]:taskBar(math.random(5000, 7500),"Oxygen Tank")
            ClearPedTasks(PlayerPedId())
            if finished == 100 then
                if hasEnoughOfItem(itemid,1,false) then
    ...
    if (itemid == "blowtorch") then
        TriggerServerEvent('drp-storerobberies:findSafe', ItemInfo.id)
    end
    ...
    if (itemid == "bandage") then
        ...
        TriggerServerEvent('drp-status:applyStatus', 'Fresh Bandaging')
    end

    if (itemid == "sbbandage") then
        TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49,10000,"Bandaging Wounds ","mythic_hospital:items:bandage",true,itemid,playerVeh)
        TriggerServerEvent('drp-status:applyStatus', 'Fresh Bandaging')
    end
    ...
    if (itemid == "gpstransponder") then
        TriggerEvent("drp-police:toggleDispatchBlips")
    end
    
    if (itemid == "unoreversecard") then 
        local ItemInfo = GetItemInfo(slot)
        TriggerEvent("drp-police:showUnoReverseCard", ItemInfo.information)   
    end

    if (itemid == "weazel_mediabadge") then
        local ItemInfo = GetItemInfo(slot)
        TriggerEvent("drp-police:showMediaBadge", ItemInfo.information)   
    end

    if (itemid == "idcard") then 
        local ItemInfo = GetItemInfo(slot)
        TriggerEvent("drp-police:showId", ItemInfo.information)   
    end

    if (itemid == "policebadge") then 
        local ItemInfo = GetItemInfo(slot)
        TriggerEvent("drp-police:showPoliceBadge", ItemInfo.information)   
    end

    if (itemid == "emsbadge") then 
        local ItemInfo = GetItemInfo(slot)
        TriggerEvent("drp-police:showEMSBadge", ItemInfo.information)   
    end
    ...
    if (itemid == "treat") then
        ...
        if model == `a_c_shepherd` then
            local t = exports["drp-progressbar"]:taskBar(2000,"Eating Treat")
            if t == 100 then
                TriggerEvent('drp-status:add', 'hunger', math.random(1625, 2000))
                TriggerEvent('drp-status:add', 'thirst', math.random(1625, 2000))
            end
        end
    end  
    ...
    if (itemid == "cleanwipes") then
        ...
        TriggerServerEvent('drp-status:removeStatus', 'Body Sweat')
        TriggerEvent("inventory:removeItem",itemid, 1)
    end
    ...
    if (itemid == "potbrownie" or itemid == "potcookie") then
        ...
        TriggerEvent('drp-status:add', 'hunger', math.random(400, 600))
        TriggerEvent('drp-status:add', 'thirst', math.random(400, 600))
        TriggerEvent("updatestress", -math.random(100,200))
        SetTimecycleModifier("DRUG_2_drive") -- BloomMid, Barry1_Stoned, DRUG_2_drive
        SetPedMotionBlur(playerPed, true)
        ClearPedTasks(PlayerPedId())
        TriggerServerEvent('drp-status:applyStatus', 'Red Eyes')
        TriggerServerEvent('drp-status:applyStatus', 'Smells Like Marijuana')
        canOpenInventory = true
    ...
    TriggerEvent('drp-carstatus:itemUsage', itemid)
    ...
    if (itemid == "legacyvipcard_gold") or (itemid == "legacyvipcard_platinum") or (itemid == "legacyvipcard_diamond") or (itemid == "legacyvipcard_legendary") then
        ...
        if ItemInfo and ItemInfo['information'] then
            ...
            if tonumber(cid) == plyData['cid'] then 
                ...
                TriggerServerEvent('drp-scripts:showCard',card,info)
            else
                exports['drp-notifications']:SendAlert('inform', 'This card is not registered to you.')
            end
        else
...
AddEventHandler('oxygentank', function()
    ...
    CreateThread(function()
        ...
        exports['drp-notifications']:SendAlert('inform', 'Oxygen tank applied at 100%', 7500)
        ...
        while true do
            ...
            if math.ceil(oxygenTime) == 22500 then
                exports['drp-notifications']:SendAlert('inform', '75% oxygen tank usage remaining', 7500)
            elseif math.ceil(oxygenTime) == 15000 then
                exports['drp-notifications']:SendAlert('inform', '50% oxygen tank usage remaining', 7500)
            elseif math.ceil(oxygenTime) == 7500 then
                exports['drp-notifications']:SendAlert('inform', '25% oxygen tank usage remaining', 7500)
            elseif math.ceil(oxygenTime) == 3000 then
                exports['drp-notifications']:SendAlert('inform', '10% oxygen tank usage remaining', 7500)
            elseif math.ceil(oxygenTime) == 1500 then
                exports['drp-notifications']:SendAlert('inform', '5% oxygen tank usage remaining', 7500)
...
end) -- exports['drp-inventory']:getInventory()

exports("canOpenInventory", function() 
    return canOpenInventory 
end) -- exports['drp-inventory']:canOpenInventory()
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh)
    ...
    if timer > 0 then
        local finished = exports["drp-progressbar"]:taskBar(timer,message,true,false,playerVeh)
...
AddEventHandler('food:breakfast', function(item)
    if item == 'eggsbacon' then
        if Breakfast() then
            if hasEnoughOfItem(item,1,false) then
                exports['drp-notifications']:SendAlert('inform', 'Good morning! What a great time to have some breakfast.', 7500)
                TriggerEvent('drp-status:add', 'hunger', math.random(2250, 2500))
            else
                print("dONT exploit me motheryacker.")
            end
        else
            if hasEnoughOfItem(item,1,false) then
                exports['drp-notifications']:SendAlert('inform', 'Good mor- oh, it\'s not... not the best time to have some breakfast.', 7500)
                TriggerEvent('drp-status:add', 'hunger', math.random(1750, 2250))
            else
    ...
    elseif item == 'fishchips' then
        if Breakfast() then
            if hasEnoughOfItem(item,1,false) then
                exports['drp-notifications']:SendAlert('inform', 'Yummy! Some Fish & Chips... are we in Britain?', 7500)
                TriggerEvent('drp-status:add', 'hunger', math.random(2500, 2750))
            else
                print("dONT exploit me motheryacker.")
            end
        else
    ...
    if playerVeh ~= 0 then
        ...
        if IsThisModelACar(model) or IsThisModelABike(model) or IsThisModelAQuadbike(model) then
            if IsEntityInAir(playerVeh) then
                Wait(1000)
                if IsEntityInAir(playerVeh) then
                    exports['drp-notifications']:SendAlert('inform', 'You appear to be flying through the air.', 5000)
                    return false
                end
            end
        end
    ...
    if not validWaterItem[itemID] and not isWeapon then
        if IsPedSwimming(player) then
            ...
            if #(targetCoords - plyCoords) > 1.3 then
                exports['drp-notifications']:SendAlert('inform', 'You cannot swim whilst using this item.', 5000)
                return false
            end
        end

        if IsPedSwimmingUnderWater(player) then
            exports['drp-notifications']:SendAlert('inform', 'You cannot be underwater and use this item.', 5000)
            return false
        end
    end
...
exports("isMethActive", isMethActive) -- exports["drp-inventory"]:isMethActive()
...
AddEventHandler('takedrugs', function(type)
    ...
    if type == "meth" then
        ...
        local f = exports["drp-progressbar"]:taskBar(5000,"Meth")
        canOpenInventory = true
        if f == 100 then
            ...
            TriggerServerEvent('drp-status:applyStatus', 'Dialated Eyes')
            TriggerServerEvent('drp-status:applyStatus', 'Body Sweat')
            TriggerServerEvent('drp-status:applyStatus', 'Labored Breathing')
    ...
    elseif type == "coke" then
        TriggerEvent("inventory:removeItem",type, 1)
        CokeAnim()
        local finished = exports["drp-progressbar"]:taskBar(9000,"Enjoying Coke")
        canOpenInventory = true
        if finished == 100 then
            ...
            if math.random(100) >= 75 then
                TriggerServerEvent('drp-status:applyStatus', 'Agitated')
            end
            Wait(math.random(45000, 75000))
    ...
    elseif type == "adderall" then
        TriggerEvent("inventory:removeItem",type, 1)
        TriggerEvent('play:anim', type)
        local finished = exports["drp-progressbar"]:taskBar(5500,"Adderall Time")
        canOpenInventory = true
        if finished == 100 then
    ...
    elseif type == "xanax" then
        ...
        local finished = exports["drp-progressbar"]:taskBar(math.random(4000, 4500),"Xanny")
        canOpenInventory = true
        if finished == 100 then
    ...
    elseif type == "lean" then
        ...
        local finished = exports["drp-progressbar"]:taskBar(math.random(7500, 8000),"Lean")
        canOpenInventory = true
        if finished == 100 then
            ...
            TriggerServerEvent('drp-status:applyStatus', 'Enlarged Pupils')
            SetPedMotionBlur(playerPed, true)
            TriggerServerEvent('drp-status:applyStatus', 'Body Sweat')
    ...
    elseif type == "opium" then
        ...
        --if hasEnoughOfItem(type,1,false) then
            local opiumyababy = exports["drp-progressbar"]:taskBar(5000,"Smoking Opium")
            canOpenInventory = true
    ...
    elseif type == "joint" then
        ...
        if not canJoint then
            exports['drp-notifications']:SendAlert('inform', 'Unable to use this just yet.', 5000)
            return
        end
        ...
        WeedAnim()
        canJoint = false
        local m = exports["drp-progressbar"]:taskBar(5000,"Smoking Joint",true,true,true)
        canOpenInventory = true
        if m == 100 then
            SetTimecycleModifier("BloomMid") -- BloomMid, Barry1_Stoned, DRUG_2_drive
            SetPedMotionBlur(playerPed, true)
            local newHealth = math.min(maxHealth , math.floor(health + maxHealth/8))
            SetEntityHealth(playerPed, newHealth)
            TriggerEvent('drp-status:remove', 'hunger', math.random(0, 0))
            TriggerEvent('drp-status:remove', 'thirst', math.random(0, 0))
    ...
    elseif type == "blunt" then
        ...
        BluntAnim()
        canJoint = false
        local b = exports["drp-progressbar"]:taskBar(5000,"Smoking Blunt",true,true,true)
        canOpenInventory = true
        if b == 100 then
            --SetTimecycleModifier("BloomMid") -- BloomMid, Barry1_Stoned, DRUG_2_drive
            ...
            local newHealth = math.min(maxHealth , math.floor(health + maxHealth/6))
            SetEntityHealth(playerPed, newHealth)
            TriggerEvent('drp-status:remove', 'hunger', math.random(300, 500))
            TriggerEvent('drp-status:remove', 'thirst', math.random(300, 500))
            TriggerServerEvent('drp-status:applyStatus', 'Red Eyes')
            TriggerServerEvent('drp-status:applyStatus', 'Smells Like Marijuana')
    ...
    elseif type == "phoenix_joint" then
        ...
        PhoenixAnim()
        canJoint = false
        local b = exports["drp-progressbar"]:taskBar(5000,"Smoking Phoenix",true,true,true)
        canOpenInventory = true
        if b == 100 then
            SetTimecycleModifier("DRUG_2_drive") -- BloomMid, Barry1_Stoned, DRUG_2_drive
            SetPedMotionBlur(playerPed, true)
            local newHealth = math.min(maxHealth , math.floor(health + maxHealth/6))
            SetEntityHealth(playerPed, newHealth)
            TriggerEvent('drp-status:remove', 'hunger', math.random(200, 400))
            TriggerEvent('drp-status:remove', 'thirst', math.random(200, 400))
            TriggerServerEvent('drp-status:applyStatus', 'Red Eyes')
            TriggerServerEvent('drp-status:applyStatus', 'Smells Like Marijuana')
    ...
    elseif type == "ecstasy" then
        TriggerEvent("inventory:removeItem",type, 1)
        local biggie = exports["drp-progressbar"]:taskBar(5000,"I warned ya")
        canOpenInventory = true
        if biggie == 100 then
            ...
            TriggerEvent('drp-status:remove', 'thirst', math.random(500, 750))
            Wait(180000 + 30000)
    ...
    elseif type == "shroom" then
        TriggerEvent("inventory:removeItem",type, 1)
        local fungitime = exports["drp-progressbar"]:taskBar(5000,"Yum, some fungi...")
        canOpenInventory = true
    ...
    elseif type == "pill_ftl" then
        ...
        local finished = exports["drp-progressbar"]:taskBar(5500,"You actually took it?!?!")
        canOpenInventory = true
        if finished == 100 then
            ...
            TriggerEvent('drp-status:remove', 'hunger', math.random(2500, 5000))
            TriggerEvent('drp-status:remove', 'thirst', math.random(2500, 5000))
...
function BluntAnim()
        ...
        while true do
            ...
            if isSmoking then
                if IsPedShooting(ped) then
                    exports['drp-notifications']:SendAlert('inform', 'Shooting and smoking, bad.', 6000)
                    isSmoking = false
                end
    ...
    CreateThread(function()
        ...
        while isSmoking do
            ...
            if shouldEnd then
                exports['drp-notifications']:SendAlert('inform', 'You finished your blunt.', 6000)
                isSmoking = false
                return
            ...
            elseif not weedCooldown and math.random(1, 2) == math.random(1, 2) then
                ...
                if math.random(1, 100) >= 75 then
                    ...
                    if closestPlayers then
                        for k,v in pairs(closestPlayers) do
                            TriggerServerEvent('drp-status:applyStatus', 'Smells Like Marijuana', GetPlayerServerId(v))
                        end
                    end
                end
...
function WeedAnim()
    ...
    CreateThread(function()
        ...
        while isSmoking do
            ...
            if shouldEnd then
                exports['drp-notifications']:SendAlert('inform', 'You finished your joint.', 6000)
                isSmoking = false
                return
            elseif not IsEntityPlayingAnim(ped, "amb@world_human_smoking_pot@male@base", 'base', 3) then
                isSmoking = false
    ...
    while isFueling do
        if #(GetEntityCoords(vehicle) - GetEntityCoords(ped)) > 2.5 then
            if isFueling then
                exports["drp-progressbar"]:closeGuiFail()
                isFueling = false
                exports['drp-notifications']:SendAlert('inform', 'You went too far away from the vehicle.', 5000)
            end
        end
        ...
        if GetPedInVehicleSeat(vehicle, -1) ~= 0 then
            if isFueling then
                exports["drp-progressbar"]:closeGuiFail()
                isFueling = false
                exports['drp-notifications']:SendAlert('inform', 'The vehicle drivers seat is occupied!', 5000)
            end
        end
        ...
        if IsEntityDead(vehicle) then
            if isFueling then
                exports["drp-progressbar"]:closeGuiFail()
                isFueling = false
                exports['drp-notifications']:SendAlert('inform', 'Somehow the vehicle is dead idk', 5000)
            end
        end
...
AddEventHandler('changehunger', function(item)
    if ChangeHungerItems[item] then
        ...
        if HungerInfo then TriggerEvent('drp-status:add', 'hunger', math.random(HungerInfo['low'], HungerInfo['high'])) end
        local ThirstInfo = ChangeHungerItems[item]['thirst']
        if ThirstInfo then TriggerEvent('drp-status:add', 'thirst', math.random(ThirstInfo['low'], ThirstInfo['high'])) end
    ...
    if item == 'chips' then
        if math.random(1,100) >= 85 then
            exports['drp-notifications']:SendAlert('inform', 'You broke the bag down into a sheet of plastic.', 6000)
            TriggerEvent('player:receiveItem', 'plastic', 1)
        end
    elseif item == 'milkshake' then
        if math.random(1,100) >= 75 then
            exports['drp-notifications']:SendAlert('inform', 'You broke the milkshake bottle down into 3 sheets of plastic.', 6000)
            TriggerEvent('player:receiveItem', 'plastic', 3)
        end
    elseif item == 'water' then
        if math.random(1,100) >= 75 then
            exports['drp-notifications']:SendAlert('inform', 'You broke the bottle down into 2 sheets of plastic.', 6000)
...
AddEventHandler('drinkalcahol', function(itemid)
    ...
    if math.random(100) >= 50 then TriggerServerEvent('drp-status:applyStatus', 'Uncoordinated') end
    if math.random(100) >= 50 then TriggerServerEvent('drp-status:applyStatus', 'Breath smells like Alcohol') end
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot,iteminfoid)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            Citizen.Wait(1000)
            TriggerServerEvent('drp-sounds:PlayWithinDistance', 3.0, 'lockpick', 0.4)
            ...
            local finished = exports["drp-skillbar"]:taskBar(25000,3)
            ...
            local finished = exports["drp-skillbar"]:taskBar(2200,10)
            ...
            TriggerServerEvent('drp-status:applyStatus', 'Has scratches on hands')
            ...
            if finished == 100 then
                ...
                if #(GetEntityCoords(targetVehicle) - GetEntityCoords(PlayerPedId())) < 10.0 and targetVehicle ~= 0 and GetEntitySpeed(targetVehicle) < 5.0 then
                    ...
                    exports['drp-notifications']:SendAlert('inform', 'Vehicle Unlocked', 3000)
                    TriggerEvent('drp-sounds:PlayOnOne', 'unlock', 0.1)
    ...
    elseif targetVehicle ~= 0 and not isForced then
        ...
        local idfk = exports["drp-lockpick"]:lockpick(pickhealth, pickdamage, pickPadding, distance)
        if math.random(100) >= 50 then TriggerServerEvent('drp-status:applyStatus', 'Has scratches on hands') end
        if idfk ~= 100 then
        ...
        if idfk == 100 then
            ...
            if #(GetEntityCoords(targetVehicle) - GetEntityCoords(PlayerPedId())) < 10.0 and targetVehicle ~= 0 and GetEntitySpeed(targetVehicle) < 5.0 then
                ...
                exports['drp-notifications']:SendAlert('inform', 'Ignition Working', 3000)
...
AddEventHandler('veh:repairing', function(itemid, targetVehicle)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
            ...
            local finished = exports["drp-skillbar"]:taskBar(15000,math.random(10,20))
            if finished ~= 100 then
                fixingvehicle = false
                reapiring = false
                ...
                if bypass then
                    ...
                    if advanced and (bypass or hasEnoughOfItem(itemid,1,false)) then
                        ...
                        if bypass or hasEnoughOfItem(itemid,1,false) then
                            ...
                            local finished = exports["drp-skillbar"]:taskBar(timer,math.random(5,15))
                            if finished ~= 100 then
                                fixingvehicle = false
                                reapiring = false
                                ClearPedTasks(playerped)
        ...
        if bypass then
            local finished = exports['drp-progressbar']:taskBar(repairlength, "Full Repair")
            if finished == 100 then
                SetVehicleEngineHealth(targetVehicle, 1000.0)
                SetVehicleBodyHealth(targetVehicle, 1000.0)
                SetVehiclePetrolTankHealth(targetVehicle, 4000.0)
                ...
                TriggerServerEvent('drp-repair:charge', math.ceil((GetEntityMaxHealth(targetVehicle) - originalHealth) / 1.5))
            end
        end
        fixingvehicle = false
    end
...
AddEventHandler("vicodin:heal", function(item)
    if not healing then
        if hasEnoughOfItem(item,1,false) then
            if item ~= 'IFAK' then
                overdoseC = overdoseC + math.random(0, 0)
            end
		...
		exports['drp-notifications']:SendAlert('inform', 'Already healing.', 2000)
	end
end)
...
AddEventHandler("vicodin:finalheal", function(item) 
	...
	local finished = exports["drp-progressbar"]:taskBar(3500, 'Healing')
    if finished == 100 then
        ClearPedTasks(PlayerPedId())
...
RegisterNetEvent('drp-washkit:onUse')
AddEventHandler('drp-washkit:onUse', function(itemid)
    if reapiring then return end
	local playerped = PlayerPedId()
    local coordA = GetEntityCoords(playerped, 1)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            local finished = exports["drp-progressbar"]:taskBar(cleanlength,"Cleaning")
            if finished == 100 then
                SetVehicleDirtLevel(targetVehicle, 0)
                exports['drp-notifications']:SendAlert('inform', 'Vehicle Cleaned.')
            end
            ClearPedTasks(playerped)
        end
        fixingvehicle = false
...
AddEventHandler('drp-robberies:client:tryrob', function(sentInfo, sentId)
    ...
    local finished = exports['drp-thermite']:startGame(dropam, latter, fspeed, inter)
    
    if finished then
        TriggerEvent('drp-robberies:client:closedoors', sentInfo['doorId'], getEntity())
        TriggerServerEvent('drp-doors:alterlockstate', sentInfo['doorId'])
    else
        if math.random(3) > 1 then
            local coords = GetEntityCoords(PlayerPedId())
            exports['drp-thermite']:startFireAtLocation(coords.x, coords.y, coords.z, math.random(5000, 10000))
        end
    end
    
    ClearPedTasks(PlayerPedId())
    TriggerEvent("destroyPropPhone")
    PaletoInfo[sentId]['status'] = 0 TriggerServerEvent('drp-robberies:server:paletobank', PaletoInfo)   
end)
...
AddEventHandler('drp-robberies:client:closedoors', function(sentId, sentEntity)
    if sentId then
        CreateThread(function()
            Wait(240000)
            local doorInfo = exports['drp-doors']:GetDoorInfo(sentId)
            if doorInfo and doorInfo['lock'] == 0 then
                TriggerServerEvent('drp-doors:alterlockstate', sentId)
...
AddEventHandler('drp-robberies:client:paletobank', function()
    for i=1, #PaletoInfo do
        if PaletoInfo[i]['type'] == 'door' and PaletoInfo[i]['status'] == 0 then
            local doorInfo = exports['drp-doors']:GetDoorInfo(PaletoInfo[i]['doorId'])
            if doorInfo then
                local coords = GetEntityCoords(PlayerPedId())
                if #(coords - doorInfo['coords']) < 1.5 then
                    TriggerServerEvent('drp-robberies:server:paletobank', PaletoInfo, i)
                    return
                end
            end
        elseif PaletoInfo[i]['type'] == 'loot' and PaletoInfo[i]['status'] == 0 then
            ...
            if #(coords - PaletoInfo[i]['coords'].xyz) < 1 then
                TriggerServerEvent('drp-robberies:server:attemptloot', i-2, exports['drp-inventory']:hasEnoughOfItem('laptop_h', 1, false))
                return
            end
        end 
    end 
...
RegisterNetEvent('drp-robberies:client:attemptloot')
AddEventHandler('drp-robberies:client:attemptloot', function(data)
    TaskPlayAnim(PlayerPedId(), "cellphone@", "cellphone_text_read_base", 2.0, 1.0, -1, 49, 0, 0, 0, 0)
    TriggerEvent("attachItemPhone","phone01")
    ...
    TriggerEvent("mhacking:start",math.random(3,6),math.random(10,15), function(success, reason)
        ...
        if success then
            if exports['drp-inventory']:hasEnoughOfItem('laptop_h', 1, false) then
                TriggerServerEvent('drp-robberies:server:getloot', data)
            end
        else
            TriggerEvent('inventory:removeItem', 'laptop_h', 1)
            TriggerServerEvent('drp-robberies:server:failedloot', data)
        end
    end)
...
RegisterNetEvent('drp-robberies:client:paletobank:adjustValue')
AddEventHandler('drp-robberies:client:paletobank:adjustValue', function(data, sentNum, sentId)
    PaletoInfo = data
    if sentId == GetPlayerServerId(PlayerId()) then TriggerEvent('drp-robberies:client:tryrob', PaletoInfo[sentNum], sentNum) end
end)]]