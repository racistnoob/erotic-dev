...
RegisterNetEvent('customEvent:turnTextRed');
onNet('customEvent:turnTextRed', (hash) => {
    // Your JavaScript code to turn the text red
    const textElement = document.getElementById('textElementId'); // Replace with the actual ID of the HTML element
...
on('inventory:removeItem', (id, amount) => {
	...
	//emit("hud-display-item", id, `<span style="color: red;">Removed</span>`, amount)
})
...
function CreateCraftOption(id, add, craft) {
	...
	//if(exports["drp-inventory"].canOpenInventory() == false){return;}
	let itemArray = [
...
on('inventory-open-request', () => {
	...
	/*if (exports['drp-housing'].inHouse()) {
		if (objectFound != 0) {
			//console.log(actualObject)
	...
	if (actualObject == -654402915) {
		...
		if (exports['drp-inventory'].hasEnoughOfItem('okaylockpick', 1, false)) {
			emitNet("server-inventory-open", startPosition, cid, "1", container);
		}
	} else if (BinFound) {
...
function GroundInventoryScan() {
	/*if (exports["drp-motels"].inApartment()) {
		emitNet("server-inventory-open", GetEntityCoords(PlayerPedId()), cid, "3", "create");
		exports["drp-notifications"].SendAlert('inform', 'Any dropped items in the free apartment will be lost.', 5000)
	}*/
