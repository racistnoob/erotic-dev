...
RegisterNetEvent('drp-inventory:search')
AddEventHandler('drp-inventory:search', function(n, i, t)
    ...
    if IsEntityPlayingAnim(me, 'random@mugging3', 'handsup_standing_base', 3) or IsEntityPlayingAnim(me, 'missarmenian2', 'drunk_loop', 3) or IsEntityPlayingAnim(me, 'mp_arrest_paired', 'crook_p2_back_right', 3) then
        exports['drp-notifications']:SendAlert('inform', 'But you\'re dead or putting your hands up?')
        return
    end
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        ...
        SpikeStrip()
        local x = exports["drp-progressbar"]:taskBar(3000,"Searching")
        if x == 100 then
...
RegisterNetEvent('drp-inventory:steal')
AddEventHandler('drp-inventory:steal', function(n, i, t, cash, targetcid)
    local closestPlayer, closestDistance =  GetClosestPlayer()
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        local searchPlayerPed = GetPlayerPed(closestPlayer)
        local me = PlayerPedId()
        ...
        if IsEntityPlayingAnim(searchPlayerPed, 'random@mugging3', 'handsup_standing_base', 3) or IsEntityPlayingAnim(searchPlayerPed, 'missarmenian2', 'drunk_loop', 3) or IsEntityPlayingAnim(searchPlayerPed, 'mp_arrest_paired', 'crook_p2_back_right', 3) then
            ...
            SpikeStrip()
            local y = exports["drp-progressbar"]:taskBar(5000,"Robbing")
            if y == 100 then
                ClearPedTasks(PlayerPedId())
                TriggerEvent("server-inventory-open", n, i, t)
...
RegisterNUICallback('StealAll', function(data, cb)
    TriggerServerEvent('drp-inventory:takeAll', data[1])
    cb('ok')
end)
...
RegisterNUICallback('checkBusinessShop', function(data, cb)
	...
	if businesssItemCheck[item] then
		...
		TriggerServerEvent('drp-inventory:payBusiness', businesssItemCheck[item], amount, cost)
	end
    cb('ok')
end)
...
local function HandleVape()
	if not isVaping then
		...
		TriggerEvent('drp-prompts:ShowUI', 'show', '[E] Take a hit<br>[G] Reset rest position')
	else
		...
		ClearPedSecondaryTask(ped)
		exports['drp-notifications']:SendAlert('inform', 'Vape stopped')
	end
end
