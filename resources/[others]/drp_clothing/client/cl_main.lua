...
local function PlayerModel(data)
	local skins = nil
	if (data['name'] == 'skin_male') then skins = frm_skins
	else skins = fr_skins end
...
function EnableGUI(enable, menu)
	...
	if (not enable) then
		...
		SetPedArmour(plyPed, armor)
		SetEntityHealth(plyPed, health)
		armor, health = 0, 0
	elseif enable then
		armor = GetPedArmour(plyPed)
		health = GetEntityHealth(plyPed)
	end
end
			...
			for categ in pairs(tattooHashList) do
					if k == categ then
							...
							if something ~= nil then
									if currentTats == nil then currentTats = {} end;
...
function Save(save)
	...
	if save then
			...
			TriggerServerEvent("drp_clothing:insert_character_current", data)
			if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
				TriggerServerEvent("drp_clothing:insert_character_face", data)
				TriggerServerEvent("drp_clothing:set_tats", currentTats)
				regmodels = true
			end
			...
			if newplayer then
					if inClothes then
						if regmodels then
							...
							inBarber = true
							OpenMenu("Barber") 
						elseif not regmodels then
							inTattoo, newplayer, enabled = false, false, false
							TriggerServerEvent('erp_newplayer:tutorial')
						end
					elseif inBarber then
							inBarber = false
							inTattoo = true
							OpenMenu("Tattoo")
					elseif inTattoo then
							inTattoo, newplayer, enabled = false, false, false
							TriggerServerEvent('erp_newplayer:tutorial')
					end
			end
...
local function FirstSpawn(kvpArmour, kvpStress, kvpThirst, kvpHunger)
	local plyPed = PlayerPedId()
	SetCanAttackFriendly(plyPed, true, false)
	NetworkSetFriendlyFireOption(true)
	SetPlayerInvincible(plyPed, false)
	SetPedArmour(plyPed, kvpArmour or 0)
	TriggerEvent('updatestress', kvpStress or 0, false)
	TriggerEvent('drp-status:set', 'hunger', kvpHunger or 5000)
	TriggerEvent('drp-status:set', 'thirst', kvpThirst or 5000)
end
...
AddEventHandler("drp_clothing:setclothes", function(data,alreadyExist,firstspawn, kvpArmour, kvpStress, kvpThirst, kvpHunger)
	plyPed = PlayerPedId()
	local currHealth = GetEntityHealth(plyPed)
	...
	if not data.model and alreadyExist ~= nil then ApplySkin(alreadyExist); return end
	model = data.model
	...
	TriggerServerEvent("drp_clothing:fromclient:get_character_face", model)
	TriggerServerEvent("drp_clothing:retrieve_tats")
	plyPed = PlayerPedId()
	SetPedArmour(plyPed, 100)
end)
...
AddEventHandler("drp_clothing:settattoos", function(playerTattoosList) currentTats = playerTattoosList SetTats(GetTats()) end)
...
local function Raycast(dist)
	...
	local entityWorld = GetOffsetFromEntityInWorldCoords(ply, 0.0, dist, 0.0)
	local ray = StartShapeTestRay(plyCoords, entityWorld, -1, ply, 1)
...
local whitelisted_to_use_custom_skins = {
...
RegisterCommand("customped", function(source, args, rawCommand)
	local found = false
	if IsNearShop(clothingShops, GetEntityCoords(PlayerPedId()), 'Clothing') < 9.0 or exports["drp-motels"]:OutfitManagement() then
		found = true
	else
	...
	if found then 
		local sentType = args[1]
		if sentType then
			local Info = whitelisted_to_use_custom_skins[sentType]
			if Info then
				local PlayerData = exports['drp']:GetPlayerData()
				if type(Info) == 'table' then
					if Info['job'] == PlayerData['job']['name'] then
						SetModelOC(GetHashKey(sentType))
					end
				elseif Info == PlayerData['cid'] then
					SetModelOC(GetHashKey(sentType))
				else
					exports['drp-notifications']:SendAlert('error', 'You lack permission to use this ped.', 5000)
				end
			else
				exports['drp-notifications']:SendAlert('error', 'This custom ped does not exist.', 5000)
			end
		else
			exports['drp-notifications']:SendAlert('error', 'You did not specify a ped.', 5000)
		end
	else
		exports['drp-notifications']:SendAlert('error', 'You are not near a wardrobe!', 5000)
	end
end, false)

RegisterNetEvent('drp_clothing:callOutfits')
AddEventHandler('drp_clothing:callOutfits', function(skincheck)
	SendNUIMessage({
        type = "outfitData",
		outfits = skincheck
	})
	...
	for i = 1, #skincheck do
		exports['drp-notifications']:SendAlert("inform", skincheck[i].slot .. " | " .. skincheck[i].name, 10000)
	end
end)

RegisterNetEvent('drp_clothing:outfits')
AddEventHandler('drp_clothing:outfits', function(pAction, pId, pName)
	if pAction == 1 then TriggerServerEvent("drp_clothing:set_outfit",pId, pName, GetCurrentPed()) -- Update/Add
	elseif pAction == 2 then TriggerServerEvent("drp_clothing:remove_outfit",pId) -- Delete
	elseif pAction == 3 then TriggerEvent('drp-sounds:PlayOnOne','Clothes1', 0.6) TriggerServerEvent("drp_clothing:get_outfit", pId) -- Set
	elseif pAction == 4 then TriggerServerEvent("drp_clothing:rename_outfit",pId, pName, GetCurrentPed()) -- Rename outfit
	end
end)
...
--[[CreateThread(function()
	while true do
		Wait(1000)
		if currentMenu ~= "" and not newplayer then
			currentCharge = currentCharge + 1
		elseif currentCharge > 0 then
			if didSave then TriggerServerEvent('drp_clothing:chargePlayer', currentCharge) end
			currentCharge = 0
			didSave = false
		end
	end
...
AddEventHandler('echorp:doLogout', function()
    if enabled then EnableGUI(false, false) end
    inClothes = false
    inBarber = false
    inTattoo = false
    armor, health = 0, 0
