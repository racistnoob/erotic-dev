...
RegisterNetEvent('erp_adminmenu:openAdminMenu', function(playerCount, maxPlayerCount)
  ...
  SendReactMessage('playerCount', playerCount) 
  SendReactMessage('maxPlayerCount', maxPlayerCount)
end)
        ...
        CreateThread(function()
            while spectateInfo['toggled'] do 
                ...
                if not DoesEntityExist(targetPed) then
                    ...
                    exports['drp-voice']:setOverrideCoords(false)
                end
            end
        end)
    end
...
CreateThread(function()
  while true do Wait(0)
      if spectateInfo['toggled'] then
          ...
          if DoesEntityExist(targetPed) then
              exports['drp-voice']:setOverrideCoords(GetEntityCoords(targetPed))
...
exports('SpectateInfo', SpectateInfo) -- exports[drp_adminmenu']:SpectateInfo()
...
AddEventHandler('erp_adminmenu:Cloak', function(from)
    if from ~= nil then
        ...
        if Cloack then
            exports['drp-notifications']:SendAlert('inform', 'You were set to invisible by: '..from..'!')
        else
            exports['drp-notifications']:SendAlert('inform', 'You were set to visible by: '..from..'.')
        end
    end
end)
...
AddEventHandler('erp_adminmenu:numcheckresources', function(id, sentNum, sentResources, moreInfo)
    if id and sentNum then
        exports['drp-notifications']:SendAlert('inform', 'You have '..GetNumResources()..' resources and '..id..' has '..sentNum..' resources loaded.', 7500)
        local resourceList = ""
...
RegisterNUICallback('toggleMouse', function()
  SetNuiFocus(false, false)
end)
