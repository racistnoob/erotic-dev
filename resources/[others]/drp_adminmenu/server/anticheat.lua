...
if DoesEntityExist(entity) then
	...
	if BlacklistModels[model] then
		...
		if model ~= -150975354 then
			...
			local webhook = "https://discordapp.com/api/webhooks/713464272310763560/mD-s1ehKYxRguViUIDFuTwG3rdTEd7LJv_7Ai8DfiwErFAvT3z393SA3qWqHs0WoBpn7"
			--sendToDiscord('Prop Created!', message, "1127128", webhook)
		end
	end
end
end)
	...
	if src then
			...
			exports.oxmysql:insert("INSERT INTO bannedplayers (name, steamid, discord, license, ip, fivem, reason, executioner, date) VALUES (:name, :steamid, :discord, :license, :ip, :fivem, :reason, :executioner, :date)", { 
					...
					if banId then
							...
							--sendToDiscord('Player Banned', message, "16711680", "https://discordapp.com/api/webhooks/759812828089155665/hIoAnPYkc7Smi_DlozDATfW9rUw9TDac2QQl9wddylM2upyGOGs5E008tGyIast4LjVq")
...
exports('BanPlayer', BanPlayer) -- exports[drp_adminmenu']:BanPlayer(src, 'Doing something weird with multichar.', 69)
...
RegisterNetEvent('esx_steam:addLicense')
AddEventHandler('esx_steam:addLicense', function(target, type, cb)
BanPlayer(target, 'Triggered esx_steam:addLicense lol kid.', 69)
end)

RegisterNetEvent('esx_steam:removeLicense')
AddEventHandler('esx_steam:removeLicense', function(target, type, cb)
BanPlayer(target, 'Triggered esx_steam:removeLicense lol kid.', 69)
end)]]